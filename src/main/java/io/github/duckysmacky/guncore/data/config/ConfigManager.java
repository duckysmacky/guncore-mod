...
import io.github.duckysmacky.guncore.network.packets.LoadConfigPacket;
import io.github.duckysmacky.guncore.network.packets.SyncConfigPacket;
...
public class ConfigManager {
    ...
    public void loadConfig() {
        if (FMLCommonHandler.instance().getSide().isServer()) {
            ...
            GuncoreMod.LOGGER.info(String.format("[%s] Requesting config from server", ID));
            PacketHandler.instance().sendToServer(new LoadConfigPacket());
        }
    }

    public void syncWithClients() {
        GuncoreMod.LOGGER.info(String.format("[%s] Syncing config with clients", ID));
    ...
    public void cacheGameConfig(String json) {
        try {
           cachedGameConfig = gson.fromJson(json, GameConfig.class);
            ...
            GuncoreMod.LOGGER.error(String.format("[%s] Error parsing JSON: %s", ID, e.getMessage()));
